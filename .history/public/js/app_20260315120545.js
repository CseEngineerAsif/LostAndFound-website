  ...
  const chatForm = document.getElementById('chat-form');
    ...
    if (typeof io !== 'undefined' && chatForm && chatBox) {
            ...
            users.forEach(user => {
                if (user.userId !== userId) {
                    ...
                    li.addEventListener('click', () => {
                        ...
                        if(chatWith) chatWith.innerText = `Chat with ${user.username}`;
        ...
        function appendMessage(sender, message) {
            ...
            messageElement.className = `d-flex flex-column ${isMe ? 'align-items-end' : 'align-items-start'} mb-2`;
            messageElement.innerHTML = `<div class="p-2 rounded ${isMe ? 'bg-primary text-white' : 'bg-white border'}" style="max-width: 80%;">${message}</div>`;
